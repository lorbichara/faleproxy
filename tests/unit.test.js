...
describe('Yale to Fale replacement logic', () => {
  ...
  test('should handle text that has no Yale references', () => {
    const htmlWithoutYale = `
      ...
      <body>
        <h1>Hello World</h1>
        <p>This is a test page with no Y a l e references.</p>
      </body>
      </html>
    `;
    ...
    expect(modifiedHtml).toContain('<p>This is a test page with no Y a l e references.</p>');
  });
  ...
  test('should handle case-insensitive replacements', () => {
    ...
    $('body *').contents().filter(function() {
      ...
      const text = $(this).text();
      // Handle case-sensitive replacements to preserve case patterns
      const newText = text
      .replace(/YALE/g, 'FALE')
      .replace(/Yale/g, 'Fale')
      .replace(/yale/g, 'fale');
